...
//  - `SwiftNIO` (default): existing SwiftNIO backend — unchanged behavior (incl. the regular-WASI
//    NIOAsyncRuntime substitution).
//  - `EmbeddedWASI`: NIO-free, Swift-Concurrency SQLite driver over CSQLite for Embedded Swift /
//    WASI. Enable it *instead of* the defaults; turns `SwiftNIO` off and drops swift-nio.
...
let package = Package(
    ...
    traits: [
        ...
        .trait(
            name: "EmbeddedWASI",
            description: "NIO-free, Swift-Concurrency SQLite driver for Embedded Swift / WASI; drops swift-nio."
        ),
    ...
    targets: [
        ...
        .target(
            ...
            dependencies: [
                ...
                // SwiftNIO backend products — only when the SwiftNIO trait is enabled.
